...
- 存在线程 ID 时，来源名称右侧显示“进入对话 ↗”，通过 Codex Desktop 已注册的 `codex://threads/<threadId>` 深链接切回对应任务；这个动作不会提交回复或关闭弹窗。
- 右上角提供最小化按钮；最小化不会解除等待，可从 Windows 任务栏恢复弹窗。
...
如果你希望 Codex 在执行每个任务时都知道可以调用这个 Skill，请打开：

**设置 → 个性化 → 自定义指令**

英文界面对应：**Settings → Personalization → Custom instructions**。

把下面这段中文原文完整粘贴进去并保存：

```text
当任务必须由用户本人介入才能继续时，例如登录、扫码、验证码、授权、提供用户专属信息或完成必须人工操作和必须要用户来确认的步骤，请调用 $codex-notifications 弹窗提醒用户。普通进度、可自行解决的错误和任务完成不要提醒。调用后不得结束当前任务；必须按 Skill 规则持续等待用户回应，若用户没有回应，至少等待20分钟，期间不能停止对话，收到有效结果后再继续原任务。执行非必要不打扰原则。
```

根据 [OpenAI 官方文档](https://learn.chatgpt.com/docs/reference/settings#personalization)，在界面中编辑“自定义指令”会更新个人级 `AGENTS.md`。因此，熟悉文件配置的用户也可以直接添加到：

- **所有项目生效：** `%USERPROFILE%\.codex\AGENTS.md`
- **仅当前项目生效：** 当前项目根目录下的 `AGENTS.md`

文件不存在时可以新建，项目内的规则优先级高于全局规则。普通用户直接使用设置界面即可。

安装程序不会自动修改你的自定义指令，以免覆盖或污染已有规则。如果你让 Codex 代为安装，安装完成后 Codex 必须把设置路径和这段原文展示给你，由你本人粘贴并保存。
...
正常情况下，你会看到一个暖白色置顶窗口：左上角显示来源，右上角提供最小化和关闭按钮。在 Codex 任务中运行测试时，来源右侧还会显示“进入对话 ↗”，点击即可切回当前任务。选择任一选项会立即提交；也可以在输入框中填写文字后点击箭头。测试弹窗两分钟后自动关闭。
